...
Le développement commence page par page avec une console web statique et sans dépendance. La première page, **Vue d’ensemble**, se trouve dans `src/operator-console/` et utilise uniquement des données synthétiques. Elle affiche la posture du parc enrôlé, des tendances de détection et un triage d’alertes, sans connexion à un appareil ni capacité de commande distante.
...
Puis ouvrir `http://localhost:8000`. Les pages suivantes seront ajoutées une par une, en commençant par l’inventaire des appareils explicitement enrôlés.
